...
export default function Edit( {route, navigation}) {
    ...
    const getDetalleVentas= async () => {
        ...
        const solicitud= await fetch(
          'http://192.168.1.4:6001/api/ventas/listarDetalle?id='+id,
          {
            method: 'GET', 
            headers: {
    ...
    const getClientes= async () => {
      ...
      const solicitud= await fetch(
        'http://192.168.1.4:6001/api/cliente/listar',
        {
          method: 'GET', 
          headers: {
            Accept: 'application/json',
...
const modificarVenta= async()=> {
  ...
  const solicitud= await fetch(
    'http://192.168.1.4:6001/api/ventas/modificar?id='+id,
    {
      method: 'PUT', 
      headers: {
        Accept: 'application/json',
    ...
    return detalleVenta.map((item)=> {
        return (
           ...
           <View style={styles.containerImagen}>
                <Image 
                style={styles.imagen}
                source={{uri: 'http://192.168.1.4:6001/api/archivos/consultar?id='+item.Productos_IdProducto}}
             ></Image>
           </View>
